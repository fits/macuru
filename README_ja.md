...
主な目的は、ボイラープレートを排除して開発体験を向上する事です。
...
```adt```マクロがenum定義等のボイラープレートコードを生成するため必要な箇所の実装に専念できます。
...
* 要素型やトレイトの型引数に```Self```、コンテナ型、ライフタイムを使用できない（例. ```Elem1<Self>```, ```Elem2<Option<T>>```）
...
リスト内包表記よりも汎用的なモナド内包表記で実現する方法を選択しました。

* Vecに```flat_map```や```and_then```等が用意されていない
* Iteratorを使うと```collect```で元の型に戻す必要が生じる

そのため、モナドそのものではなく、あくまでも内包表記の実現にフォーカスしています。
